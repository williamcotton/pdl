...
Successor plan: [`V0_33_PLAN.md`](V0_33_PLAN.md)
Related Algraf plan: [`V0_69_PLAN.md`](../../algraf/docs/V0_69_PLAN.md)
...
PDL v0.32 starts the native execution performance work by adding an
...
The v0.32 release should introduce the layer that lets native execution keep
...
The neighboring Algraf v0.69 performance plan owns the consumer-side work:
...
- Add the first native Polars fast path for path-backed pipelines.
  ...
  The v0.32 native fast path should cover the lowest-risk stages:
...
- `cargo run -p pdl-bench -- run --suite large --run-label <label>` writes
  timestamped CSV rows to `bench/runs/<label>/report.csv` so before/after
  large-data runs can be compared with the same report contract used by
  `algraf-bench`.
...
- Keep spec, versions, and release documents aligned when implementation lands.
  ...
  - Workspace/package version stamps that track the release are aligned to
    `0.32.0` when this numbered plan is implemented.
...
- Pilot grouped aggregate native coverage if the core fast path is stable.
  ...
  be included in v0.32 only after null behavior, output ordering, and type
...
- PDL-to-Algraf smoke testing aligned with
  [`V0_69_PLAN.md`](../../algraf/docs/V0_69_PLAN.md) when a local Algraf binary
  is available.
- Benchmarks at small, medium, and large data sizes:
  1,000 rows, 100,000 rows, and 1,000,000 or more rows.
- `pdl-bench` exercises checked-in large `.pdl` workload programs against
  ignored generated/downloaded fixtures and writes CSV report rows with status,
  output path, input/output row counts where available, byte count, elapsed
  milliseconds, run timestamp, and git ref.
