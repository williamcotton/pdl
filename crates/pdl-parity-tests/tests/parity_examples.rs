...
// through `pdl run` on row, row-strict, auto, and native/native-strict where
// fixtures pin native selection. The row engine is the parity spec: stdout
// payloads and saved/named-output files from the other engine legs must match
// it. CSV and JSON Lines are
...
/// end-to-end); `auto` must match whichever engine it selects; forced
/// `native` and `native-strict` legs run only for examples the fixtures pin
/// as natively executed.
fn candidate_engines(name: &str) -> Vec<&'static str> {
    let mut engines = vec!["row-strict", "auto"];
    if expected_selected_engine(name) == "native" {
        engines.push("native");
        engines.push("native-strict");
    }
    engines
}
