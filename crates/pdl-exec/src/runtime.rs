            ...
            match stage {
                ...
                StageIr::Unsupported { name, span } => {
                    return Err(Diagnostic::error(
                        codes::E1211,
                        format!("stage `{name}` is deferred in 0.19.0"),
                        *span,
                    ));
                }
            }
...
fn resolve_input_format(
    ...
    if let Some(format) = explicit_format {
        return DataFormat::from_name(format).ok_or_else(|| {
            Diagnostic::error(
                codes::E1215,
                format!("format `{format}` is not supported in 0.19.0"),
                span,
            )
        });
    }
    if matches!(&input.source, SourceDescriptor::Stdin) {
        if let Some(format) = stdin_format {
            return DataFormat::from_name(format).ok_or_else(|| {
                Diagnostic::error(
                    codes::E1215,
                    format!("stdin format `{format}` is not supported in 0.19.0"),
                    input.span,
                )
            });
        }
...
fn resolve_output_format(
    ...
    if let Some(format) = explicit_format {
        return DataFormat::from_name(format).ok_or_else(|| {
            Diagnostic::error(
                codes::E1705,
                format!("output format `{format}` is not supported in 0.19.0"),
                span,
            )
        });
    }
